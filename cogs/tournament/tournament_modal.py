...
import config
from cogs.models.nickname_and_id_separation import nickname_and_id_separation, link_plus_nickname_and_id, nicknames_to_string
...
class TournamentModal(disnake.ui.Modal):
    def __init__(self, type: str, bot):
        ...
        components = [
            disnake.ui.TextInput(label = "Название команды", placeholder = "Введите название команды", custom_id = "team_name"),
            disnake.ui.TextInput(label = "Капитан команды", placeholder = "Ник, discord id", custom_id = "captain"),
            disnake.ui.TextInput(label = "Состав команды", placeholder = "Ник, discord id, ник, discord id и т.д", custom_id = "members"),
            disnake.ui.TextInput(label = "id участников в игре", placeholder = "Через запятую, в том же порядке", custom_id = "game_id"),
            disnake.ui.TextInput(label = "Ранги участников", placeholder = "Через запятую, в том же порядке", custom_id = "ranks"),
        ]
    ...
    async def callback(self, interaction: disnake.ModalInteraction):
        ...
        if captain_nickname_and_id:
            ...
            embed.add_field(name = '> Капитан команды:', value = f"```{interaction.text_values['captain']}```", inline = False)
        ...
        if members_game_id:
            ...
            embed.add_field(name = '> id участников в игре:', value = f"```{interaction.text_values['game_id']}```", inline = False)
